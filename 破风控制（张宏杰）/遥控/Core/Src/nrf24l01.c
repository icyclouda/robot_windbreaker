...
const unsigned char INIT_ADDR0[5]= {0x01,0x3A,0xB1,0xB1,0x01};
...
void RX_Mode(void)
{
	Clr_NRF24L01_CE;
	//写RX节点地址
	NRF24L01_Write_Buf(SPI_WRITE_REG+RX_ADDR_P0,(unsigned char*)INIT_ADDR1,RX_ADR_WIDTH);
	...
	//设置RF通信频率
	NRF24L01_Write_Reg(SPI_WRITE_REG+RF_CH,50);
	//选择通道0的有效数据宽度
...
void TX_Mode(void)
{
  //启动发送模式
	Clr_NRF24L01_CE;
	//写TX节点地址
	NRF24L01_Write_Buf(SPI_WRITE_REG+TX_ADDR,(unsigned char*)INIT_ADDR1	,TX_ADR_WIDTH);
	//设置TX节点地址,主要为了使能ACK
	NRF24L01_Write_Buf(SPI_WRITE_REG+RX_ADDR_P0,(unsigned char*)INIT_ADDR1,RX_ADR_WIDTH);
	...
	NRF24L01_Write_Reg(SPI_WRITE_REG+SETUP_RETR,0x15);//5
	//设置RF通道为40
	NRF24L01_Write_Reg(SPI_WRITE_REG+RF_CH,50);//50
	//设置TX发射参数,0db增益,2Mbps,低噪声增益开启
	NRF24L01_Write_Reg(SPI_WRITE_REG+RF_SETUP,0x0f);
